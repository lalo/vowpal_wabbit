    ...
    if (model_file.num_files() > 0)
    {
      ...
      if (all.model_file_ver < VERSION_FILE_WITH_INTERACTIONS_IN_FO)
      {
        ...
        for (size_t i = 0; i < pair_len; i++)
        {
          ...
          if (std::count(all._interactions.begin(), all._interactions.end(), temp) == 0)
          { all._interactions.emplace_back(temp.begin(), temp.end()); }
        }
        ...
        for (size_t i = 0; i < triple_len; i++)
        {
          ...
          if (count(all._interactions.begin(), all._interactions.end(), temp) == 0)
          { all._interactions.emplace_back(temp.begin(), temp.end()); }
        }
            ...
            VERSION_FILE_WITH_INTERACTIONS)  // && < VERSION_FILE_WITH_INTERACTIONS_IN_FO (previous if)
        {
          ...
          for (size_t i = 0; i < len; i++)
          {
            ...
            if (count(all._interactions.begin(), all._interactions.end(), temp) == 0)
            { all._interactions.emplace_back(buff2, buff2 + inter_len); }
          }
