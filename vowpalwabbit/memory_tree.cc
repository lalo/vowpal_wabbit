...
base_learner* memory_tree_setup(VW::setup_base_fn& stack_builder)
{
  ...
  if (tree->oas == false)
  {
    ...
    learner<memory_tree, example>& l = init_multiclass_learner(tree, as_singleline(stack_builder.setup_base_learner()),
        learn, predict, all.example_parser, num_learners, all.get_setupfn_name(memory_tree_setup));
  ...
  else
  {
    ...
    learner<memory_tree, example>& l = init_learner(tree, as_singleline(stack_builder.setup_base_learner()), learn,
        predict, num_learners, prediction_type_t::multilabels, all.get_setupfn_name(memory_tree_setup));
