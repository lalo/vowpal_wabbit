...
VW::LEARNER::base_learner* test_reduction_setup(VW::setup_base_i& setup_base, VW::config::options_i& options, vw& all)
{
  ...
  auto ret = VW::LEARNER::make_no_data_reduction_learner(
      as_singleline(base), predict_or_learn<true>, predict_or_learn<false>, all.get_setupfn_name(test_reduction_setup))
